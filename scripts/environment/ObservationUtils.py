...
class GazeboObservationGenerator:
    ...
    def get_occupancy_grid(self):
        ...
        for name, pose in zip(model_states.name, model_states.pose):
            if 'wall' in name:  # replace 'wall' with the actual names of your wall models
                x = pose.position.x
                y = pose.position.y
                min_x = min(min_x, x)
                min_y = min(min_y, y)
        ...
        for name, pose in zip(model_states.name, model_states.pose):
            ...
            x, y = self.pose_to_grid_coordinates(pose, min_x, min_y)
            grid[y][x] = 1

        return grid

    def pose_to_grid_coordinates(self, pose, min_x, min_y):
...
def test():
    ...
    generator = GazeboObservationGenerator('robot')  # replace 'robot' with the actual name of your robot
    ...
    for relative_location, relative_orientation in relative_locations_and_orientations:
        print(f'Relative Location: {relative_location}, Relative Orientation: {relative_orientation}')
